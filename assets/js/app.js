...
`).join('');
 $('#video-posts').innerHTML=d.videos.map(p=>{const id=yt(p.youtube_url);return `<article class="card video">${id?`<iframe loading="lazy" src="https://www.youtube-nocookie.com/embed/${esc(id)}" title="${esc(p.title)}" allowfullscreen></iframe>`:'<div class="monogram">▶</div>'}<p class="date">${esc(p.date)}</p><h3>${esc(p.title)}</h3><p>${esc(p.description)}</p></article>`}).join('');
}).catch(()=>{});
